...
@section('content')
    ...
    <script>
        ...
        const AUTOSAVE_KEY = 'ventaPersistenteCotizacion';
        const CLIENTE_KEY = 'clientePersistenteCotizacion';
        ...
        function guardarVentaPersistente() {
            try {
                ...
                sessionStorage.setItem('ticketGuardadoCotizacion', JSON.stringify(ticket));
                if (clienteActual && clienteActual.id) {
                    sessionStorage.setItem('clienteGuardadoCotizacion', JSON.stringify(clienteActual));
                }
        ...
        function limpiarVentaCompletada() {
            try {
                ...
                sessionStorage.removeItem('clienteGuardadoCotizacion');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            restaurarDatosCotizacion(); // Agregar función específica para cotizaciones
        });

        function restaurarDatosCotizacion() {
            // Restaurar datos de cotización si viene desde clientes
            if (sessionStorage.getItem('navegandoDesdeCotizacion') === 'true') {
                // Restaurar productos de cotización
                const cotizacionGuardada = sessionStorage.getItem('cotizacionGuardada');
                if (cotizacionGuardada) {
                    try {
                        const productos = JSON.parse(cotizacionGuardada);
                        if (productos && productos.length > 0) {
                            // Asignar a la variable global de cotizaciones
                            if (typeof productosEnCotizacion !== 'undefined') {
                                productosEnCotizacion = productos;
                                if (typeof actualizarTablaCotizacion === 'function') {
                        ...
                        sessionStorage.removeItem('cotizacionGuardada');
                    } catch (error) {
                ...
                // Restaurar cliente
                const clienteGuardado = sessionStorage.getItem('clienteGuardadoCotizacion');
                if (clienteGuardado) {
                    try {
                        ...
                        console.error('Error al restaurar cliente de cotización:', error);
                        sessionStorage.removeItem('clienteGuardadoCotizacion');
                    }
                }

                // Verificar si hay un cliente seleccionado desde la vista de clientes
                const clienteSeleccionadoClientes = sessionStorage.getItem('clienteSeleccionadoCotizacion');
                if (clienteSeleccionadoClientes) {
                    try {
                        const cliente = JSON.parse(clienteSeleccionadoClientes);
                        if (typeof clienteActual !== 'undefined') {
                            clienteActual = cliente;
                            document.getElementById('footer-cliente').textContent = cliente.nombre;
                            ...
                            if (typeof mostrarNotificacion === 'function') {
                                mostrarNotificacion(`Cliente "${cliente.nombre}" seleccionado para la cotización`);
                            }
                        }
                        sessionStorage.removeItem('clienteSeleccionadoCotizacion');
                    } catch (error) {
                ...
                // Restaurar otros datos de cotización
                const datosCotizacion = sessionStorage.getItem('datosCotizacion');
                if (datosCotizacion) {
                    try {
                        const datos = JSON.parse(datosCotizacion);
                        if (datos.vigencia && document.getElementById('vigencia-dias')) {
                            document.getElementById('vigencia-dias').value = datos.vigencia;
                            document.getElementById('footer-vigencia').textContent = datos.vigencia + ' días';
                        }
                        if (datos.observaciones && document.getElementById('observaciones-input')) {
                            document.getElementById('observaciones-input').value = datos.observaciones;
                        }
                        sessionStorage.removeItem('datosCotizacion');
                    } catch (error) {
                        console.error('Error al restaurar datos de cotización:', error);
                        sessionStorage.removeItem('datosCotizacion');
                    }
                }

                // Limpiar la marca de navegación desde cotización
        ...
        function verificarClienteSeleccionado() {
            ...
            if (ticket.length === 0) {
                const ticketGuardado = sessionStorage.getItem('ticketGuardadoCotizacion');
                if (ticketGuardado) {
                    try {
            ...
            const clienteData = sessionStorage.getItem('clienteSeleccionadoCotizacion');
            if (clienteData) {
                try {
        ...
        // Función para navegar a clientes marcando que viene desde COTIZACIONES
        function navegarAClientes() {
            // Guardar los productos de la cotización actual antes de navegar
            if (ticket && ticket.length > 0) {
                sessionStorage.setItem('cotizacionGuardada', JSON.stringify(ticket));
                if (clienteActual && clienteActual.id) {
                    sessionStorage.setItem('clienteGuardadoCotizacion', JSON.stringify(clienteActual));
                }
                // Guardar otros datos de la cotización
                const vigencia = document.getElementById('vigencia-dias') ? document.getElementById('vigencia-dias').value : 30;
                const observaciones = document.getElementById('observaciones-input') ? document.getElementById('observaciones-input').value : '';
                sessionStorage.setItem('datosCotizacion', JSON.stringify({
                    vigencia: vigencia,
        ...
        function emitirVentaConTipo(tipoDocumento) {
            ...
            form.method = 'POST';
            form.action = '{{ route("cotizaciones.emitir.post") }}';
    ...
    <script>
        ...
        function resetearSistemaCompleto() {
            ...
            localStorage.removeItem('ticketCotizacion');
            localStorage.removeItem('ticketGuardadoCotizacion');
            localStorage.removeItem('clienteGuardadoCotizacion');
