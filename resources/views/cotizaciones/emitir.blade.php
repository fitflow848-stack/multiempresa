...
<div style="max-width:1200px;margin:20px auto;border:1px solid #ddd;padding:16px;background:#fff;">
    ...
    @if(isset($clienteData) && $clienteData)
        <div class="cliente-info">
            ...
            @if(json_decode($clienteData)->numero_documento ?? null)
                - {{ json_decode($clienteData)->tipo_documento ?? 'DNI' }}: {{ json_decode($clienteData)->numero_documento }}
            @endif
        </div>
    @else
        <div class="cliente-info">
    ...
    <div style="display:flex;gap:20px;">
        <div style="flex:1;border-right:1px solid #eee;padding-right:12px;">
            <div style="font-size:18px;font-weight:600;margin-bottom:8px;">TOTAL <span style="float:right;font-size:22px;">S/ <span id="total-amount">{{ $total ?? '0.00' }}</span></span></div>

            <div style="margin:12px 0;">
                <label>ENTREGA</label>
                <input id="entrega" type="number" step="0.01" value="{{ $total ?? '0.00' }}" style="width:100%;padding:8px;margin-top:6px;border:1px solid #bcd;" onkeyup="calcularCambio()">
            </div>
        ...
        <div style="flex:1;padding-left:12px;">
            ...
            <div style="margin-top:12px;display:flex;gap:8px;">
                <input type="text" id="guia-remision" placeholder="Guía Remisión Rem." style="flex:1;padding:8px;border:1px solid #bcd;">
                <input type="text" id="guia-transporte" placeholder="Guía Remisión Trans." style="flex:1;padding:8px;border:1px solid #bcd;">
            </div>

            <div style="margin-top:12px;display:flex;gap:8px;align-items:center;">
                <div style="width:60px;">Serie</div>
                <input type="text" id="serie" value="{{ $serieDocumento ?? ($company->serie_boleta ?? 'B001') }}" style="width:80px;padding:8px;border:1px solid #bcd;" readonly>
                <input type="text" id="numero" value="0001" style="flex:1;padding:8px;border:1px solid #bcd;" readonly>
            </div>
...
function cancel(){
    if (confirm('¿Está seguro que desea cancelar? Se perderán todos los datos.')) {
        window.location = '{{ route('cotizaciones.create') }}';
    }
}
