    ...
    public function buscar(string $q, ?int $sucursalId = null, bool $includeEmpty = false): array
    {
        ...
        return DB::select("
            SELECT
                ...
                (SELECT d2.costo FROM almacen_ingreso_detalle d2 WHERE d2.id = MAX(ad.id)) AS costo,
                (SELECT d2.pvp FROM almacen_ingreso_detalle d2 WHERE d2.id = MAX(ad.id)) AS pvp,
                (SELECT d2.pvpd FROM almacen_ingreso_detalle d2 WHERE d2.id = MAX(ad.id)) AS pvpd,
                (SELECT d2.pvc FROM almacen_ingreso_detalle d2 WHERE d2.id = MAX(ad.id)) AS pvc,
                (SELECT d2.pvcd FROM almacen_ingreso_detalle d2 WHERE d2.id = MAX(ad.id)) AS pvcd,
