    ...
    public function buscar(string $q, ?int $sucursalId = null, bool $includeEmpty = false): array
    {
        ...
        return DB::select("
            SELECT
                ...
                (SELECT sub.pvp FROM almacen_ingreso_detalle sub WHERE sub.producto_id = ad.producto_id AND sub.producto_linea_id = ad.producto_linea_id AND sub.pvp > 0 ORDER BY sub.id DESC LIMIT 1) AS pvp,
                (SELECT sub.pvpd FROM almacen_ingreso_detalle sub WHERE sub.producto_id = ad.producto_id AND sub.producto_linea_id = ad.producto_linea_id AND sub.pvpd > 0 ORDER BY sub.id DESC LIMIT 1) AS pvpd,
                (SELECT sub.pvc FROM almacen_ingreso_detalle sub WHERE sub.producto_id = ad.producto_id AND sub.producto_linea_id = ad.producto_linea_id AND sub.pvc > 0 ORDER BY sub.id DESC LIMIT 1) AS pvc,
                (SELECT sub.pvcd FROM almacen_ingreso_detalle sub WHERE sub.producto_id = ad.producto_id AND sub.producto_linea_id = ad.producto_linea_id AND sub.pvcd > 0 ORDER BY sub.id DESC LIMIT 1) AS pvcd,
